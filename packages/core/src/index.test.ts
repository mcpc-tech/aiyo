...
describe("AiyoAdapter (high-value unit tests)", () => {
  ...
  describe("JavaScript code execution plugin", () => {
    ...
    it("suspends on the first sandbox tool call and exposes the bridge tool call", async () => {
      ...
      vi.mocked(generateText).mockResolvedValueOnce(
        makeCodeExecutionResult(
          "code_exec_1",
          ['const data = await tools.read_file({ path: "/tmp/test.txt" });', "return data;"].join(
            "\n",
          ),
        ),
      );

      const adapterWithPlugin = createCodeExecutionAdapter(["read_file", "write_file", "list_dir"]);
    ...
    it("resumes sandbox completion and feeds the final result back through the original conversation", async () => {
      ...
      expect(res2.choices[0].message.content).toBe("The file contains hello world");
    ...
    it("can suspend more than once and return the next bridge tool call on each resume", async () => {
      ...
      const adapterWithPlugin = createCodeExecutionAdapter(["read_file", "list_dir"]);
      ...
      const secondResponse = await adapterWithPlugin.handleChatCompletion(
        createResumeRequest("inspect the project", firstBridgedCall, '{"dir":"/tmp/project"}'),
      );
      ...
      const finalResponse = await adapterWithPlugin.handleChatCompletion(
        createResumeRequest("inspect the project", secondBridgedCall, '["a.txt","b.txt"]'),
      );
  ...
  it("streams responses events in OpenAI SDK compatible order", async () => {
    ...
    expect(chunks.some((chunk) => chunk.includes("event: response.output_item.added"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: response.content_part.added"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: response.output_text.delta"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: response.output_text.done"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: response.output_item.done"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: response.completed"))).toBe(true);
    expect(chunks[chunks.length - 1]).toBe("data: [DONE]\n\n");
  });
  ...
  it("streams Anthropic SSE events for messages", async () => {
    ...
    expect(chunks.some((chunk) => chunk.includes("event: content_block_start"))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes('"type":"text_delta"'))).toBe(true);
    expect(chunks.some((chunk) => chunk.includes("event: message_delta"))).toBe(true);
